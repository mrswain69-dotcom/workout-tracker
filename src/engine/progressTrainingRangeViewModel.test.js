...
function range({ label, startDate = "", endDate = "", completed = 0, minutes = 0, attempts = 0, successes = 0, accuracyPct = null, count = 0 } = {}) {
  return {
    label,
    startDate,
    endDate,
    ...
    trend: completed
      ? [{ startDate: startDate || "2026-09-01", endDate: endDate || "2026-09-10", completedSessions: completed, totalMinutes: minutes }]
      : [],
    sessionBalance: [
...
describe("Phase 3 Stage 7 training range view model", () => {
  ...
  it("keeps range summary, Session mix and Movement totals aligned to one selection", () => {
    const model = buildTrainingRangeViewModel({
      ...
      month: range({ label: "This month", startDate: "2026-09-01", endDate: "2026-09-30", completed: 1, minutes: 10, count: 1 }),
      lifetime: range({ label: "All time", completed: 4, minutes: 65, count: 4 }),
    });
