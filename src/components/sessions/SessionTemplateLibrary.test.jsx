...
vi.mock("./SessionTemplateEditor.jsx", async () => {
  ...
  return {
    default: function MockSessionTemplateEditor({
      ...
      const rows = templateMovements?.length
        ? templateMovements
        : [
            {
      ...
      const nextTemplate = {
        ...template,
        name: existing ? `${template.name} Edited` : "New Session",
      };
      ...
      return ReactModule.createElement(
        ...
        ReactModule.createElement(
          "button",
          {
            type: "button",
            disabled: saving,
            onClick: () => onSave?.({ template: nextTemplate, templateMovements: rows }),
          },
          "Save Mock Session"
        ),
        ReactModule.createElement(
...
function makeDbApi(seed = seedState()) {
  ...
  return {
    state,
    loadSessionLibrary: vi.fn(async () => ({ data: snapshot(), error: null })),
...
describe("SessionTemplateLibrary", () => {
  it("loads the Stage 2 definition library and lists Programmes and Sessions", async () => {
    ...
    expect(screen.getAllByText("Football Skills").length).toBeGreaterThanOrEqual(1);
  ...
  it("creates and edits Programmes through the Stage 2 DB API", async () => {
    ...
    await waitFor(() =>
      expect(db.createProgramme).toHaveBeenCalledWith(
        "f1",
        expect.objectContaining({ name: "Ball Mastery", category: "Football" })
      )
    );
    await waitFor(() => expect(screen.getAllByText("Ball Mastery").length).toBeGreaterThan(0));
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save Programme" }));
    await waitFor(() =>
      expect(db.updateProgramme).toHaveBeenCalledWith(
        expect.any(String),
  ...
  it("creates a Session inside the selected Programme through the controller", async () => {
    const db = await renderLibrary();
  ...
  it("edits an existing Session and bumps its template version", async () => {
    const db = await renderLibrary();
  ...
  it("archives a Session after confirmation and reloads the library", async () => {
    const db = makeDbApi();
  ...
  it("creates and edits canonical Movements through the Stage 2 DB API", async () => {
    ...
    await waitFor(() =>
      expect(db.createMovement).toHaveBeenCalledWith(
        "f1",
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save Movement" }));
    await waitFor(() =>
      expect(db.updateMovement).toHaveBeenCalledWith(
        expect.any(String),
  ...
  it("archives an unused Movement and an empty Programme", async () => {
    const db = await renderLibrary();
    fireEvent.click(screen.getByRole("button", { name: "Movements" }));
  ...
  it("blocks archiving a Programme while it owns active Sessions", async () => {
    const db = await renderLibrary();
  ...
  it("surfaces library-load errors without writing data", async () => {
    const db = makeDbApi();
    db.loadSessionLibrary.mockResolvedValueOnce({
      data: null,
      error: new Error("network down"),
    });
    render(<SessionTemplateLibrary familyId="f1" dbApi={db} confirmArchive={() => true} />);
